...
def main():
  ...
  if output == 'y':
    ...
    if square_table == 'y':
      ...
      print("Invalid input.")
      exit(1)
    color_min = input(str("For the heatmap color scale, what should be the minimum? "))
    color_max = input(str("For the heatmap color scale, what should be the maximum? "))
    #check 0<=0min<max<=1
  ...
  norm_mode = raw_input(str("Specify which time segment to normalize over, baseline or the specific comparison segment [b/s]: "))
