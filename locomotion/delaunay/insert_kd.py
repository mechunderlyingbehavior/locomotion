...
from locomotion.delaunay.tds import box, ccw, apex, orig, dest, InfiniteVertex, \
    Vertex, Triangle, Triangulation
from locomotion.delaunay.preds import orient2d, incircle
from locomotion.delaunay.cdt import ConstraintInserter
from locomotion.delaunay.iter import FiniteEdgeIterator, StarEdgeIterator
from locomotion.delaunay.inout import output_vertices, output_triangles
...
#        print(id(pivot.triangle), "to be removed")
    ...
    def flip31(self, vertex):
        ...
        tri = vertex.triangle
        side0 = locomotion.vertices.index(vertex)
        apex0, orig0, dest0 = apex(side0), orig(side0), dest(side0)

        # the two triangles that will be removed
        ngb_orig = locomotion.neighbours[orig0] # neighbour tri opposite orig point
        ngb_dest = locomotion.neighbours[dest0] # neighbour tri opposite dest point
        ...
        if link_orig:
            tip_orig = link_orig.vertices[dest(link_orig.vertices.index(locomotion.vertices[dest0]))]
            locomotion.vertices[apex0] = tip_orig
        if link_dest:
            tip_dest = link_dest.vertices[orig(link_dest.vertices.index(locomotion.vertices[orig0]))]
            locomotion.vertices[apex0] = tip_dest
        ...
        if link_orig:
            self.link_2dir(tri, orig0, link_orig, orig(link_orig.vertices.index(locomotion.vertices[dest0])))
        else:
            self.link_1dir(tri, orig0, None)
        # link neighbours tri to link_dest
        if link_dest:
            self.link_2dir(tri, dest0, link_dest, dest(link_dest.vertices.index(locomotion.vertices[orig0])))
        else:
...
#        return candidate
    ...
    def straight_walk(self, ini, p):
        ...
        q = locomotion.vertices[q_idx]
        if orient2d(locomotion.vertices[r_idx], locomotion.vertices[q_idx], p) < 0:
            while orient2d(locomotion.vertices[l_idx], locomotion.vertices[q_idx], p) < 0:
                neighbour = locomotion.neighbours[r_idx]
                q_idx = neighbour.vertices.index(q)
                r_idx = ccw(q_idx)
                l_idx = ccw(r_idx)
                tri = neighbour
                assert locomotion.vertices[q_idx] == q
        else:
            while True:
                neighbour = locomotion.neighbours[l_idx]
                q_idx = neighbour.vertices.index(q)
                r_idx = ccw(q_idx)
                l_idx = ccw(r_idx)
                tri = neighbour
        ...
        # perform the walk
        s_idx = q_idx
        while orient2d(p, locomotion.vertices[r_idx], locomotion.vertices[l_idx]) < 0.0:
            neighbour = locomotion.neighbours[s_idx]
            l_idx = neighbour.vertices.index(locomotion.vertices[l_idx])
            r_idx = ccw(l_idx)
            s_idx = ccw(r_idx)
            tri = neighbour
            # 'advance' 1 of the two sides of the neighbour triangle
            # by swapping either l or r with s
            if orient2d(locomotion.vertices[s_idx], q, p) < 0.0:
