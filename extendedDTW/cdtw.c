...
//double ext(double *x, double *y, int n, int m, double *cost, int metric)
double ext(double *x, double *y, int n, int m, int k, double *cost )
//k is numCol of the matrix X, which also the number of variables the metric function uses
{
  int i, j;
  ...
  cost[0] = (*dist)(&x[0], &y[0], k);
  
  for (i=1; i<n; i++)
    cost[i*m] = (*dist)(&x[i*k], &y[0], k) + cost[(i-1)*m];
  
  for (j=1; j<m; j++)
    cost[j] = (*dist)(&x[0], &y[j*k], k) + cost[(j-1)];
  
  for (i=1; i<n; i++)
    for (j=1; j<m; j++)
      cost[i*m+j] = (*dist)(&x[i*k], &y[j*k], k) + 
	                  min3(cost[(i-1)*m+j], cost[(i-1)*m+(j-1)], cost[i*m+(j-1)]);
	                  //min3(w*cost[(i-1)*m+j], cost[(i-1)*m+(j-1)], w*cost[i*m+(j-1)]);
...
//k is numCol of the matrix X, which also the number of variables the metric function uses
{
  ...
  cost[0] = (*dist)(&x[0], &y[0], k);
  cost[m+1] = (*dist)(&x[k], &y[k], k) + cost[0];
  cost[m+2] = (*dist)(&x[k], &y[2*k], k) + cost[m+1];
  cost[m+3] = (*dist)(&x[k], &y[3*k], k) + cost[m+2];
  cost[2*m+1] = (*dist)(&x[2*k], &y[k], k) + cost[m+1];
  cost[2*m+2] = (*dist)(&x[2*k], &y[2*k], k) + cost[m+1];
  cost[2*m+3] = (*dist)(&x[2*k], &y[2*k], k) + cost[m+1]; 
  cost[3*m+1] = (*dist)(&x[3*k], &y[k], k) + cost[2*m+1];
  cost[3*m+2] = (*dist)(&x[3*k], &y[k], k) + cost[2*m+1]; 
